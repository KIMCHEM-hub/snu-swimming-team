...
- **관리자 초대(`create_member_account`) 플로우 버그 — 미해결(2026-08-15 발견)**: 초대 메일 링크 클릭 시 Supabase가 자동 로그인 세션을 생성하지만, 실제로 테스트해보면 "초기 비밀번호 설정" 화면(`invitePasswordView`, §"Invite initial password setup" 참고)이 뜨지 않는다. 코드 자체(`js/members.js:6-7`의 `type=invite` 감지, `:333-346`의 `pendingInviteFor()`)는 존재하지만 실제 초대 콜백에서 트리거되지 않는 것으로 관찰됨 — 원인 미확인(콜백 URL 형식이 코드가 기대하는 `?type=invite`/해시 `type=invite`와 다를 가능성 등, 추후 조사 필요). 결과적으로 사용자는 로그인된 채로 남지만 비밀번호가 없어 이후 로그아웃 시 재로그인이 불가능(비밀번호 재설정 플로우를 타야만 함). 신규 부원 초대 시마다 재발 예상. → 수정 방향: 초대 링크로 첫 로그인 시 비밀번호 설정 폼이 실제로 뜨도록 트리거 조건 재조사(또는 온보딩 리다이렉트 방식으로 전환). 우선순위는 "다음 작업" §8 참고.
