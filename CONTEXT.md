...
- **"관리자 탭 구조 개선" 미해결 메모는 해결됨** — 아래 "관리자 탭 서브탭 패턴 적용 완료" 항목 참고.

**관리자 탭 서브탭 패턴 적용 완료(2026-08-16)**
- 코치 탭에서만 쓰이던 `initCoachSubtabs()`를 `initSubtabs(panel, tabKeys)`로 일반화해 코치 패널(3탭, 동작 그대로)과 관리자 패널(6탭: NEW MEMBER ACCOUNT/MEMBER STATUS/INVITE WHITELIST/팝업 관리/출석 우수자 팝업/자가기록 승인 대기)에 동일 패턴 적용. 탭바·버튼 클래스를 기존에 재사용하던 `.members-coach-actions`/`.members-edit-button`(폼 하단 버튼 행과 공유되던 클래스)에서 전용 클래스 `.members-subtabs`/`.members-subtab`으로 분리 — 새 active 탭 스타일이 무관한 버튼 행에 새어나가는 것을 원천 차단. "수정 요청 관리"(h2/안내문/상태/요청목록)는 애초에 `.members-coach-section`으로 안 감싸여 있어 탭 시스템 밖 고정 헤더로 코드 변경 없이 유지됨(동작만 검증). i18n 키 6개(`members.adminTab{Account,Status,Whitelist,Popups,Winners,SelfReports}`) KR/EN 추가. 활성 탭 시각 표시(코치 탭엔 원래 없던 기능)를 사이트 전역 `.filter-bar`/`.record-tabs` 밑줄 패턴(`--snu-blue` 밑줄, 골드 미사용)으로 신규 추가 — 코치 탭의 "active 표시 없음" 기존 결함도 이 참에 함께 해소됨. `.members-subtabs`는 `overflow-x:auto`로 좁은 화면에서 탭 줄바꿈/잘림 방지.
- **구현 중 발견·수정한 버그**: `initSubtabs()`가 `render()` 클로저를 반환만 하고 초기화 시점엔 호출하지 않아, 첫 렌더링에서 탭 라벨이 빈 문자열로 뜨는 문제를 브라우저 실측(`textContent` 확인)으로 발견 — `render()`를 반환 전에 동기 1회 호출하도록 수정해 해결.
...
- **진행 순서 후보**(2026-08-15 갱신 — "좁은 뷰포트에서 nav 줄바꿈" 항목 제거: `963cf98` 커밋으로 이미 해결 완료된 사항(§8 기록)이며, 320px~1440px 전구간 iframe 재검증 결과 문제 재현 안 됨을 확인해 이 메모가 갱신 안 된 stale 항목이었음이 드러남):
  ...
  4. ~~멤버 대시보드~~ — **골드 오용 3건 + 관리자 탭 서브탭 패턴 적용까지 전부 완료(2026-08-16, 상세는 §8 참고)**
- **다듬을 디테일 후보**:
...
## 활동점수 및 월별 상품 — 확정 스펙 (2026-08-16)

> 아직 미구현. 위 원문의 상충·모호했던 지점(100% 구간 상품 금액 불일치, 자가기록 상한, 3인훈련 인증 방식)을 사용자와 확정한 최종 스펙. 구현 시 이 스펙을 기준으로 한다. 코드/스키마 변경 없음, 기록만.

### 점수 체계
- 정규훈련 참석 1점/회
- 자유수영 인증 0.5점
- 3인 이상 훈련표대로 이행 0.75점
- 부원 4인 이상 사적 모임 인증 0.5점

### 자가기록 일일 상한 (카테고리 묶음 방식)
- 그룹 A(자유수영 + 3인 이상 훈련표 이행): 둘을 합쳐서 하루 1건만 인정(같은 날 자유수영 했으면 3인훈련은 인정 안 됨, 반대도 동일)
- 그룹 B(4인 이상 사적 모임): 별개로 하루 1건 인정
- 즉 하루 최대 2건(그룹 A에서 1건 + 그룹 B에서 1건)까지 자가기록 점수 인정 가능

### 월별 총점 계산
- 월별 총점 = 정규훈련 점수(`attendance_type` 가중치 합) + 승인된 자가기록 점수 합
- 월별 % = 월별 총점 / 그달 정규훈련 세션 수 × 100 (기존 `monthly_attendance_rates()` 로직 그대로 재사용, 상한 캡 없음 — 120% 이상도 그대로 계산되어야 함)

### 상품 등급 (최종 확정 — 전체점수 표 기준)
- 80% 이상: 기본(유지 기준 충족, 상품 없음)
- 100% 이상: 깁티(1만원 상당)
- 120% 이상: 깁티(2만원 상당)
- 월별 1등(위 등급 상품과 중복 불가): 상품 미정, 등급만 우선 기록

### "3인 이상 훈련표 이행" 인증 방식
- 체크박스로 표시 + 참여자 명단(3인 이상) 함께 기록
...
- "3인훈련" 카테고리 참여자 명단 저장할 필드/구조 추가 필요(기존 테이블엔 없음)
- `monthly_attendance_rates()`에 자가기록 점수 합산 LEFT JOIN 추가
- 신규 테이블 필요: `monthly_prizes`(또는 `attendance_rewards`) — `member_id`, `year_month`, `tier`, `score` 스냅샷, `distributed` 등
- "1등" 판정 로직: 80/100/120% 티어 해당자를 제외한 나머지 중 최고점자 선정 규칙 신규 필요
- 관리자 UI: 월별 순위/확정 화면 신규 필요(관리자 서브탭에 추가 위치 검토)

## 출석 점수 시스템 — 실행 설계 확정 (2026-08-16, 이전 스펙 일부 개정)

이전 "확정 스펙"에서 아래 3가지를 개정함:

1. **월별 1등 정의 변경**: 기존 "80/100/120 티어 해당자를 제외한 나머지 중 최고점"에서 → **"그달 전체 최고점자(티어 무관)"**로 변경. 티어 상품과 중복 여부는 상품 자체가 미정이므로 지금 결정 안 함 — monthly_prizes에 tier='winner' 행만 기록. 동점자는 공동 1등으로 전원 기록 후 관리자가 화면에서 수동 조정(삭제/유지).

2. **Worker cron 백로그 이동**: 월간 자동 마감/확정/팝업 자동생성 cron은 지금 구현 안 함. 사유: 부원 규모 작아 월 1회 관리자 수동 확정으로 충분, cron(Cloudflare cron trigger 신규 + service role worker 신규 배포)의 설정·보안·디버깅 비용 대비 이득 낮음. 실사용 후 필요 시 재검토.

3. **확정 주체 = 관리자 수동**: 월별 시상은 관리자가 "월별 시상" 서브탭에서 연/월 선택 → 계산 결과 확인 → "확정" 버튼으로 monthly_prizes 스냅샷. 자동 확정 없음.

### 구현 순서 (미착수, B-1은 진행 중)
- B-1: 3인훈련 자가기록 UI (진행 중 — 머지 대기)
- B-2: 정규훈련 출석 일괄입력 UI — 기존 "훈련 평가 입력" 단일 폼을 교체. 세션 선택 → coach_member_directory() 활성 부원 전체를 행으로 렌더 → 각 행 출결 4택(출석/지각/인정결석/미인정결석)+코멘트 → 지정된 행만 training_evaluations upsert(onConflict: session_id,member_id). unique 제약 이미 존재(training_evaluations_session_member_key), 스키마 변경 없음. 주의: 코치 RLS 정책이 created_by 기준이라 여러 코치가 같은 세션 분할 입력하는 시나리오는 범위 밖(코치 1명이라 현재 무관).
- B-3: 관리자 "월별 시상" 서브탭 신규 — monthly_attendance_rates(year,month)로 부원별 이름/세션수/self_report_score/최종% + 티어 자동판정(≥120/≥100/≥80/미달) + 전체 최고%자 1등 뱃지 → "확정" 버튼 → monthly_prizes insert(unique 제약으로 재확정 시 중복 방지).

## 전체 UI 시각적 위계 개선 (신규, 2026-08-16 요청)

배경: 지금까지의 디자인 리프레시(HOME/TEAM/RECORDS·GALLERY·NOTICES/멤버 대시보드/관리자 탭)는 색상 토큰 통일(골드=성과 전용, line-height, 보더 색 등) 위주였음. 이번 요청은 그것과 별개로 "전체적으로 시각적 위계/강조가 부족해서 중요한 게 안 눈에 띄는 느낌" — 구조나 기능 변경 아님, 순수 시각적 강조/위계 개선.

범위: 공개 페이지 전체(HOME/TEAM/RECORDS/GALLERY/NOTICES) + 로그인 후 대시보드 전체(멤버/코치/관리자 화면 각각).

방향:
- 어떤 요소가 "지금 중요한 것"인지 한눈에 구분되도록 — 타이포 크기/굵기 대비, 여백 리듬, 색상 강조(기존 골드=성과 전용, 네이비=구조 원칙 유지하며) 등을 재검토
- 구조/기능/콘텐츠 변경 없음, 시각적 강조 방식만 조정
...
3인훈련 참여자 자동반영 완료(2026-08-16): `participant_ids uuid[]` 체크박스 방식(제출자 포함 3명 이상), `approve_self_report()` `SECURITY DEFINER` 함수로 승인 시 참여자 팬아웃(하루상한/FK 위반 시 개별 스킵). SQL 검증 완료(가짜 참여자 스킵 확인), UI 최소인원 검증 완료(활성부원 1명뿐이라 실제 3인 시나리오는 미검증 — 부원 가입 후 재확인 필요). 코치 표/부원 요약 `self_report_score` 노출도 이번 커밋에 포함.

정규훈련 출석 일괄입력 UI 완료(2026-08-16): 코치 "훈련 평가 입력"을 세션당 부원 하나씩 개별입력에서 전체 부원 일괄입력(upsert, `onConflict: session_id,member_id`)으로 교체. 기존 `training_evaluations` unique 제약 재사용, 스키마 변경 없음. UI 동작 확인 완료(세션 선택→행 렌더→기존값 프리필→저장→재조회). 출석률 현황 표는 `currentYearMonth()` 기준 당월만 조회하는 기존 동작 그대로(다른 달 데이터는 표시 안 됨, 설계상 정상).

**남은 3단계 UI**:
- 월별 순위/티어 확정 관리자 화면 없음

관리자 월별 시상 서브탭 완료(2026-08-16): 연/월 선택 → `monthly_attendance_rates()` 조회 → 클라이언트에서 티어(80/100/120) 자동판정 + 전체 최고점자 1등 뱃지(동점 공동1등) → 확정 버튼으로 `monthly_prizes` upsert. stale-data 방지(select값과 로드시점 값 불일치 시 재조회 요구) 포함. `monthly_prizes` 관리자 delete 정책 추가로 확정 취소 기능도 지원. UI 전체 시나리오 검증 완료. 알려진 이슈: 좁은 화면에서 표 컬럼이 잘림(반응형 QA 항목에서 함께 처리 예정).

1등/100%/120% 티어 달성자 축하 팝업 생성 기능 추가(2026-08-16): 월별 시상 표에서 해당 행 '팝업 만들기' 버튼 → 인라인 폼 → `popups`(type='attendance_winner') insert. `member_id` 기준 매칭(`member_name` 문자열 매칭이 아님, RECORDS 이름매칭 버그 교훈 반영). 실사용 검증 완료. 알려진 정상 동작: team.json에 프로필 없는 인물(예: 관리자 계정)이 팝업 대상이면 사진이 학교 로고로 대체됨 — 사진 업로드 통합(백로그 5순위) 완료 후 실제 부원 대상 재검증 권장.
...
## 작업 우선순위 재정렬 (2026-08-16, Claude.ai 설계 세션 기준)

가입 임박(2~3일 내) 및 노력 대비 효과 기준으로 재배치. 이전 "온 더 호라이즌" 순위 목록은 이 섹션으로 대체됨:

Tier 1 (가입 전 안전망): 세션 만료+로그인 연장 버튼(10분 목표) → MFA(관리자 계정) → Rate limit 대시보드 확인

Tier 2 (콘텐츠 기반): 사진 업로드 통합(프로필+레거시) → 부원별 출결/자가기록 전체 이력 조회 화면(관리자용, 신규)

Tier 3: CAPTCHA

Tier 4 (훈련 기록 시스템, 신규): DB 스키마(rep별 기록, 설계 확정) → 코치 입력 UI → 부원 그래프 대시보드

Tier 5 (규모 늘어날 때): 출석 일괄입력 아코디언 방식(20명+) → leadership.json memberId 연동

Tier 6 (학기 전환 임박 시): members.status(active/ob) 일괄 전환

Tier 7 (기능 다 쌓인 뒤 한번에): 크로스 디바이스 반응형 QA → 전체 UI 시각적 위계 개선

독립: Google Search Console sitemap 제출(사용자 직접, 언제든)

백로그: Worker cron(필요성 낮음, 재검토 보류)

**다음 작업은 Tier 1의 "세션 만료 + 로그인 연장 버튼"이다. UI 위계 개선은 Tier 7, 아직 착수 시점 아님.**

## 백로그 실사 대조 및 재정렬 (2026-08-16, 코드 직접 확인 기준)

이전 "작업 우선순위 재정렬" 섹션의 Tier 목록을, 실제 코드베이스와 대조해서 갱신함. 문서에 미완료로 남아있던 항목 중 실제로는 이미 구현·병합 완료된 것들을 발견:

**이미 완료로 확인됨 (문서 갱신 누락이었음):**

- 세션 만료 + 로그인 연장 버튼: SESSION_TIMEOUT_MS=10분, SESSION_WARNING_MS=2분 경고, 탭간 동기화(storage 이벤트), pageshow/visibilitychange 재계산까지 전부 구현됨 (`js/members.js` 214-300줄 부근). `members.html`에 경고배너+연장버튼 마크업도 있음. 실사용 테스트 여부만 미확인 — 다음 세션에 브라우저에서 10분 대기 검증 권장.
- 프로필 사진 업로드: `profile-photos` Supabase Storage 버킷 업로드, 미리보기, input 비활성화 처리까지 구현됨 (`js/members.js` profilePhotoInput 관련 코드).
- 레거시 사진 업로드: 관리자 "Legacy 사진 관리" 폼으로 이미 완성(요청→승인 플로우).
- 회원 상태 개별 전환: `set_member_status()` 함수로 개별 active/ob 전환 가능. 단 "여러 명 한번에" 일괄 전환 UI는 없음 — 이 부분만 남은 작업.

**확인 결과 실제로 미착수:**

- MFA, CAPTCHA: 코드 전무
- 훈련 기록 시스템(rep별 기록): 스키마 파일 자체 없음, 설계만 되어있고 미실행
- 부원별 출결/자가기록 전체이력 조회 화면(관리자용): 없음
- 아코디언 방식 출석 일괄입력: 없음
- leadership.json memberId 연동: 없음
- 학기 "일괄" OB 전환: 없음 (개별 전환은 있음, 대상 축소됨)

**갱신된 Tier 순서:**

Tier 1: MFA → CAPTCHA

Tier 2: 훈련 기록 시스템(DB→코치UI→부원그래프) → 부원별 출결 전체이력 조회 화면(신규)

Tier 3: 학기 일괄 OB 전환 (개별 전환 위에 "여러 명 선택" UI만 추가하면 됨, 스코프 작음)

Tier 4: 아코디언 방식 일괄입력(20명+ 될 때) → leadership.json memberId 연동

Tier 5: 크로스 디바이스 반응형 QA → 전체 UI 시각적 위계 개선

독립: Google Search Console sitemap 제출(사용자 직접)
...
부원 그래프 대시보드 완료(2026-08-16): "훈련 평가 및 입력" 탭에 "기록 추이" 섹션 추가. 종목/거리 조합 선택 → training_set_times를 session_id 기준으로 그룹핑(날짜 아님 — 동일 날짜 복수 세션 엣지케이스 방지) → 세션별 평균시간을 순수 SVG 라인차트로 렌더(createElementNS 기반, 차트 라이브러리 없음). 실사용 검증 완료.

훈련 기록 시스템(DB→코치UI→부원그래프) 전체 완료. Tier 2 완전 종료.

Tier 3(학기 일괄 OB 전환)을 백로그로 이동. 사유: 개별 전환(set_member_status)이 이미 있고, 부원 규모가 작아 일괄 선택 UI의 실익이 낮음. 실제로 부원 수가 늘어나서 개별 토글이 번거로워지면 그때 재검토.

다음 시작점은 Tier 4: 아코디언 방식 출석 일괄입력(20명+ 될 때) → leadership.json memberId 연동. 이후 Tier 5(반응형 QA, UI 위계 개선).

메인화면 반응형 버그 원인 확인(2026-08-16, 코드 미적용): `.hero`의 `min-height:100vh`가 고정 헤더(`.nav` height: 78px, 모바일 68px)를 고려 안 해서, 창 "높이"가 특정 임계값 아래로 줄면(폭이 아니라 높이가 트리거) 히어로 로고/타이틀이 헤더 밑에 겹침. `.nav-menu` 자체(폭 기반 오버플로우)는 문제없음, 버그 아님으로 확인됨.

제안된 수정(미적용): `.hero{min-height:calc(100vh - 78px)}`, `@media(max-width:980px)`에서 68px로 조정. 다음 세션에서 이 수정 적용 및 검증부터 시작할 것.

## Tool alternation / 컨텍스트 관리

컨텍스트 관리: 기능 하나 완성+머지 완료 시점마다 `/compact`로 컨텍스트 압축. 완전히 다른 종류 작업으로 전환할 때(예: DB작업→UI버그조사)는 `/clear`로 아예 새로 시작. claude-in-chrome 같은 MCP 브라우저 자동화 사용 직후에도 `/compact` 권장(툴 결과가 컨텍스트에 계속 남아 비용 커짐). 2026-08-16 세션에서 이걸 안 지켜서 하루 $25.47 소요됨 — 다음부터는 위 습관 적용.
